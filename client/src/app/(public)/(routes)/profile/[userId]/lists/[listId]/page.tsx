...
export async function generateMetadata({
	...
	return constructMetadata({
		list: data.list,
		pageUrl: absolutePath(`${Routes.PROFILE}/${params.userId}${Routes.LISTS}/${params.listId}`),
		title: `List: ${data.list.name} | Curated by ${data.list.creator.username}`,
		description:`${storiesCount === 0 ? 'No Stories' : storiesCount === 1 ? `${storiesCount} story` : storiesCount > 1 &&  `${storiesCount} stories`} · Description`
	})
}
